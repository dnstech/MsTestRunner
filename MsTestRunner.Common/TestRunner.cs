...
namespace MsTestRunner.Common
{
    ...
    public sealed class TestRunner
    {
        ...
        private readonly Dictionary<Assembly, List<TestItem>> assemblyTestList = new Dictionary<Assembly, List<TestItem>>();
        ...
        public void AddTestAssembly(string filePath)
        {
            ...
            if (this.testAssemblyFileNames.Add(Path.GetFullPath(filePath)))
            {
                try
                {
                    var assemblyToLoad = Path.GetFullPath(filePath);
                    Directory.SetCurrentDirectory(Path.GetDirectoryName(assemblyToLoad));
                    var assembly = Assembly.LoadFrom(assemblyToLoad);
                    var testList = new List<TestItem>();
                    this.assemblyTestList.Add(assembly, testList);
                    ...
                    this.AddTestClasses(testList, testClasses, deploymentsByType);
                }
        ...
        public void AddTestClasses(IList<TestItem> testList, IEnumerable<Type> testClassTypes, Dictionary<Type, Tuple<string, List<Tuple<string, string>>>> deploymentsByType)
        ...
        public TestRunResult Execute()
        {
           ...
           // Directory.SetCurrentDirectory(this.path);
            ...
            result.Start();
            foreach (var assemblyList in this.assemblyTestList)
            {
                Directory.SetCurrentDirectory(Path.GetDirectoryName(assemblyList.Key.Location));
                Parallel.ForEach(
                assemblyList.Value,
                new ParallelOptions
                {
